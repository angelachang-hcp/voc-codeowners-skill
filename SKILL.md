...
**Argument form.** The `codeowners` DSL and `with_codeowners` accept either a symbol (`codeowners :voice_of_customer`) or the method call (`codeowners CodeOwners.voice_of_customer`); `CodeOwners.ensure_team_name` resolves both via `public_send`. The symbol form is the prevailing house style. Examples below use the method-call form for explicitness — either is correct.

**Example annotations.** The `# ←` / `// ←` comments in the examples are documentation only. Do NOT copy them into generated code.
...
The activation conditions and the Behavior rules section below apply identically to Pattern 5.
